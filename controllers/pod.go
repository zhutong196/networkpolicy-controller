...
func (ntpc *NetworkPolicyController) newPodEventHandler() cache.ResourceEventHandler {
	return cache.ResourceEventHandlerFuncs{
		AddFunc: func(obj interface{}) {
			if podObj, ok := obj.(*api.Pod); ok {
				...
				if isOpenNetworkPolicy(podObj) {
					ntpc.OnPodUpdate(obj)
				}
			}
		},
		UpdateFunc: func(oldObj, newObj interface{}) {
			...
			// 此处不检查pod网络策略是否开启 因为网络策略的变动并不会让pod更新 此处只检查pod和networkpolicy相关属性是否发生改变
			// 改变则往fullSyncRequestChan 管道发送数据
			if isPodUpdateNetPolicyRelevant(oldPodObj, newPodObj) {
				ntpc.OnPodUpdate(newObj)
			}
		},
		DeleteFunc: func(obj interface{}) {
			ntpc.handlePodDelete(obj)
		},
	}
}

// 判断pod是否开启网络策略  不是完成状态 && 存在podIp && 不是hostnetwork类型
func isOpenNetworkPolicy(pod *api.Pod) bool {
	return !isFinished(pod) && pod.Status.PodIP != "" && !pod.Spec.HostNetwork
}

// OnPodUpdate handles updates to pods from the Kubernetes api server
func (ntpc *NetworkPolicyController) OnPodUpdate(obj interface{}) {
	pod := obj.(*api.Pod)
